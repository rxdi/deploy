...
Error loading file ${filePath}
    ...
    completeBuildAndAddToIpfs(folder, file, message, namespace, outputConfigName, buildFolder = './build') {
        ...
        return rxjs_1.from(this.parcelBuild(path_1.normalize(folder + '/' + file), buildFolder, `${file.split('.')[0]}.js`)).pipe(operators_1.tap(() => {
...
export class Pesho {
    ...
    pushTransaction(folder, file, message, namespace, outputConfigName, buildFolder = './build') {
        let ipfsFile;
        let ipfsModule;
        let ipfsTypings = this.initIpfsModule;
        let ipfsMessage = this.initIpfsModule;
        ...
        file = file.replace(/^.*[\\\/]/, '');
        return rxjs_1.from(this.parcelBuild(path_1.normalize(folder + '/' + filePathFromRepo), `${folder}/${buildFolder}`, `${file.split('.')[0]}.js`)).pipe(operators_1.tap(() => {
            this.logger.log('Bundling finished!\n');
            this.logger.log(`Adding commit message ${message}...\n`);
        }), operators_1.switchMap(() => __awaiter(this, void 0, void 0, function* () { return this.createCommitMessage(message); })), operators_1.tap(res => {
            ipfsMessage = res;
            this.logger.log(`Commit message added...\n`);
        }), operators_1.switchMap(() => this.fileService.readFile(`${folder}/${buildFolder}/${file.split('.')[0]}.js`)), operators_1.tap(() => {
            ...
            this.logger.log(`Typescript definitions merge started!\n`);
        }), operators_1.switchMap(() => rxjs_1.from(this.typingsGenerator.mergeTypings(namespace, path_1.normalize(folder + filePathFromRepo.substring(0, filePathFromRepo.lastIndexOf("/")).replace('.', '')), path_1.normalize(`${folder}/${buildFolder}/index.d.ts`)))), operators_1.tap(() => this.logger.log(`Typescript definitions merge finished! Reading file...\n`)), operators_1.switchMap(() => this.fileService.readFile(`${folder}/${buildFolder}/index.d.ts`)), operators_1.tap(() => this.logger.log(`Typescript definitions read finished! Adding to IPFS...\n`)), operators_1.switchMap((res) => {
            if (!!res) {
                return this.ipfsFile.addFile(res);
            }
            else {
