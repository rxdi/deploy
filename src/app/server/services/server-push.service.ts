...
export class ServerPushService implements PluginInterface {
  ...
  constructor(
    ...
    // );
    const interval = nextOrDefault('--server-push-interval', 1000 * 7, a =>
      Number(a * 1000)
    );
    timer(0, interval)
      ...
      .pipe(
        ...
        tap(() =>
          this.openService.openPage(
            `http://${this.server.info.address}:${this.server.info.port}/webui`
          )
        )
      )
      .subscribe();
  ...
  async stopServerWatcher() {
    return await new Promise(resolve =>
      this.serverWatcher.close(() => resolve())
    );
  }

  private createServerWatcher() {
    this.serverWatcher = createServer(this.OnRequest.bind(this));
    this.serverWatcher.listen(
      nextOrDefault('--webui-server-watcher-port', 8957)
    );
  }
